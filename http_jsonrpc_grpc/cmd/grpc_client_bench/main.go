...
func makeSomeRequestsAndMarkTheTime(ctx context.Context, client grpc_storage.FileStorageClient, requestAmount uint) error {
	timeStart := time.Now()
	for i := uint(0); i < requestAmount; i++ {

		if _, err := client.SaveToken(ctx,
			&grpc_storage.Token{
				Idjwt:     100500,
...
func main() {
	...
	grcpConn, err := grpc.Dial(
		gServerConf.Ip + ":" + gServerConf.Port,
		grpc.WithInsecure(),
	)
	...
	client := grpc_storage.NewFileStorageClient(grcpConn)
	ctx := context.Background()
	

	fmt.Printf("%sTry to send %d requests in 1 stream%s\n", GREEN, 100, NO_COLOR)
	if err := makeSomeRequestsAndMarkTheTime(ctx, client, 100); err != nil {
		log.Printf("Error while initializing configuration: %v", err)
		os.Exit(-1)
	}
