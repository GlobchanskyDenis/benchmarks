...
var (
	globalparser ConfigParser
)

type ConfigParser interface {
	ParseConfig(packStruct interface{}, blockName string) error
}
...
// Создаем парсер конфига
func NewConfigParser(fileName string) (ConfigParser, error) {
	conf := configparser{}
	var err error
	conf.bConf, err = ioutil.ReadFile(fileName)
	if err != nil {
		return nil, err
	}
	err = conf.parseConfigData()
	return &conf, err
}

// получаем данные из json
func (conf *configparser) parseConfigData() error {
	err := json.Unmarshal(conf.bConf, &conf.confmap)
	if err != nil {
		return err
	}
	if conf.confmap["common"] != nil {
		var ok bool
		conf.commonmap, ok = conf.confmap["common"].(map[string]interface{})
		if !ok {
...
*/
func (conf *configparser) ParseConfig(packStruct interface{}, blockName string) error {
	var err error
	...
	if V, ok := conf.confmap[pack]; ok {
		err = conf.switchSetType(structVal, V, structVal.Type(), "", pack_path, reflect.Value{})
	} else {
...
func SetConfigFile(fileName string) error {
	var err error
	globalparser, err = NewConfigParser(fileName)
	return err
}

// ##Заполнение структуры конфига (глобальный конфиг для совместимости)
/*
	packStruct - указатель на структуру конфига
	blockName - имя блока в json конфиге, если передать пустым, в качестве блока берется имя пакета в котором определена структура packStruct
	В json файле может быть определен необязательный блок common. В случае, если поле для структуры не найдено в блоке для конкретного пакета,
...
func ParsePackageConfig(packStruct interface{}, blockName string) error {
	return globalparser.ParseConfig(packStruct, blockName)
}

// Рекурсивная функция заполнения полей конфига
func (conf *configparser) switchSetType(field reflect.Value, json_value interface{}, field_type reflect.Type, field_tag reflect.StructTag, pack_path string, map_key reflect.Value) error {
	var err error
	...
	case reflect.Slice:
		if json_value != nil {
			...
			for j := 0; j < v_slice.Len(); j++ {
				...
				err := conf.switchSetType(slice.Index(j), v_slice.Index(j).Interface(), t_slice, field_tag, pack_path, reflect.Value{})
				if err != nil {
					return err
				}
			}
	...
	case reflect.Map:
		if json_value != nil {
			...
			for _, k := range v_map.MapKeys() {
				...
				err := conf.switchSetType(field, val_json.Interface(), field_type.Elem(), field_tag, pack_path, n_key)
				if err != nil {
					return err
				}
			}
	...
	case reflect.Struct:
		...
		for i := 0; i < field_type.NumField(); i++ {
			...
			if json_value_child == nil && field_type_child.Kind() != reflect.Ptr {
				...
				json_value_child = conf.commonmap[tag]
			...
			err = conf.switchSetType(field.Field(i), json_value_child, field_type_child, field_tag_child, pack_path, reflect.Value{})
			if err != nil {
				return err
			}
		}
	case reflect.Ptr:
		if json_value != nil {
			...
			err = conf.switchSetType(field.Elem(), json_value, field_type_child.Elem(), field_tag, pack_path, reflect.Value{})
			if err != nil {
				return err
			}
		}
